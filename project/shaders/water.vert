...
void main() {
    ...
    float bump = sin(uTime + aTextureCoord.x * uTextureScale) * sin(uTime + aTextureCoord.y * uTextureScale);
    position.xyz += aVertexNormal * bump * uScaleFactor;

    vec4 vertex = uMVMatrix * position;

    vTextureCoord = aTextureCoord * uTextureScale + vec2(uTime);
